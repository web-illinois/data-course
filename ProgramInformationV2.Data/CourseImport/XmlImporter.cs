...
namespace ProgramInformationV2.Data.CourseImport {
    ...
    public static class XmlImporter {
        ...
        public static IEnumerable<string> GetCourses(string rubric) {
            var returnValue = new List<string>();
            var urls = new List<string>();
            foreach (var year in BuildYearRange()) {
                foreach (var semester in _semesters) {
                    urls.Add($"{_scheduleUrl}{year}/{semester}/{rubric.ToUpperInvariant()}.xml");
                }
            }
            foreach (var url in urls) {
                try {
                    ...
                    foreach (var node in document.Descendants("course")) {
                        if (!returnValue.Contains(node.Attributes("id").First().Value)) {
                            returnValue.Add(node.Attributes("id").First().Value);
                        }
                    }
                } catch (WebException) {
                    Console.WriteLine($"Web Exception for {url}");
                    // do nothing -- assume a 404 error or 502 error, which means the course does not exist
                } catch (HttpRequestException) {
                    Console.WriteLine($"Http Request Exception for {url}");
                    // do nothing -- assume a 404 error or 502 error, which means the course does not exist
                }
            }
            return returnValue;
